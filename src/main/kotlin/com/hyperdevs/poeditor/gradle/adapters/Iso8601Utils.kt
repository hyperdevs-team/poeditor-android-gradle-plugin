...
internal object Iso8601Utils {
                ...
                if (date.length > offset) {
                    ...
                    if (c != 'Z' && c != '+' && c != '-') {
                        ...
                        if (checkOffset(date, offset, '.')) {
                            ...
                            val parseEndOffset = min(endOffset.toDouble(), (offset + 3).toDouble()).toInt() // parse up to 3 digits
                            val fraction = parseInt(date, offset, parseEndOffset)
                            milliseconds = (10.toDouble().pow((3 - (parseEndOffset - offset)).toDouble()) * fraction).toInt()
                            offset = endOffset
                        }
                    }
                }
     ...
     */
    @Throws(NumberFormatException::class) private fun parseInt(value: String, beginIndex: Int, endIndex: Int): Int {
